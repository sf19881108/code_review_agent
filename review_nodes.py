...
三个节点的返回值分别是 {"static_result": ...}、{"security_result": ...} 和 {"style_report": ...}，每个节点只写回自己对应的状态字段，不触碰其他字段。LangGraph 的默认 reducer 会用新值直接覆盖旧值，由于三个节点写入的是不同的字段，不存在冲突，整个合并过程对开发者完全透明。
