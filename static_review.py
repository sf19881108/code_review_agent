...
"""
实现静态分析工具。在真实的 CI/CD 环境中，静态分析通常会对接 pylint、flake8、SonarQube 等成熟的第三方工具，这些工具能够解析 AST、检测代码异味和计算圈复杂度。
在这个教学实现中，我们使用 Python 标准库的 ast 模块对代码进行真实的结构分析，同时保留了对接外部工具的接口注释，方便读者在实际项目中替换为生产级方案。
