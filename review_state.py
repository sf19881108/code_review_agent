...
将三个审查工具串联执行虽然简单，但会浪费大量的等待时间，安全扫描等待静态分析完成，风格审查又等待安全扫描完成，端到端的延迟是三者耗时之和。
...
code 字段存储用户提交的待审查代码，在分发节点中被读取后分发给三个审查节点。static_result、security_result 和 style_report 分别接收三个审查节点的输出。最后 final_report 存储汇总生成的完整审查报告。
